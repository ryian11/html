...
def sound(n):
    ...
    for r in wb[sheet].iter_rows(values_only=True):
        ...
        if pg not in pages:
            pages[pg] = {}
            order.append(pg)
        pages[pg].setdefault(key, []).append((seq, SPEAK.sub('', txt).strip(), fid))
    for pg in pages:
        for k in pages[pg]:
...
def syntax_sentences(n):
    ...
    for i, r in enumerate(ws.iter_rows(values_only=True), 1):
        if i == 1 or not r[0] or not r[1]:
            continue
        out.setdefault(str(r[0]).strip(), []).append(norm(r[1]))
    return out
...
def assemble(n, only=None):
    ...
    sub = 0
    for pi, pg in enumerate(pglist):
        key = 'p%s_%02d' % (pg, 2 if pi == 0 else 1)
        ORDER.append(key)
        ...
        # 구문 번호 |n|
        for k, sent in enumerate(syn.get(pg, []), 1):
            tgt = keytxt(sent)
            done = False
...
def render(n, d):
    L = ['# -*- coding: utf-8 -*-',
         '# %d단원 Reading 본문 페이지 데이터' % n,
         '# read_import.py 가 만든 초안입니다. `# ???` 가 붙은 곳을 확인하세요.',
         '# 출처: 녹음 대본 %d과 / 스토리보드 Lesson %d / 지도서 각론%d' % (n, n, n),
         '', 'PAGES = {']
    for page in d['ORDER']:
        p = d['PAGES'][page]
        t = p['title']
