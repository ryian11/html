...
> 1차 근거(팝업 번호·마크업·mp3 규칙의 원 분석)는 `문서/10_Reading본문페이지_규칙.md`.
> 프로젝트 전반은 `PROJECT.md`, project_rules.json 구조는 `SCHEMA.md`. 여기에는 **규칙 자체**만 적는다.
...
### 중요한 구분 (2026-09-22 확정 — 자세한 내용은 `SCHEMA.md`)

`project_rules.json`은 **프로젝트를 찾아가는 지도 + 그 프로젝트에서 발견한 규칙**(프로젝트마다 1개)이다.

`rules.json`은 **이 recipe가 HTML을 만드는 방법**(recipe마다 1개, 여러 프로젝트가 재사용)이다. 프로젝트 고유 정보(스토리보드 시트 이름 4종·`sound.sheet` 템플릿)는 `project_rules.json` 쪽으로 옮기는 것이 확정됐지만, 이 문서 1·2장의 표는 **아직 코드를 고치기 전 현재 상태**를 그대로 적어 둔 것이다 — 코드 반영 후에는 이 표도 갱신해야 한다.
