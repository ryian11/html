...
def promote(rid, lesson, keys=None, fresh=False):
    """프로토 분석 결과에서 고른 항목을 rules.json 으로 옮긴다.

    keys 가 없으면 전부. 이미 있는 항목은 덮어쓰되 예전 값을 was 로 남긴다.
    fresh=True 면 예전 rules.json 을 버리고 이번 분석 결과만 남긴다.
    """
    ...
    return {'moved': moved, 'same': same, 'path': rules_path(rid)}
