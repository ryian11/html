...
def _run_one(recipe, unit, slots, steps, out, say, pages=None):
    ...
    if 'build' in steps and not out:
        try:
            b = runner.backup(ctx, unit)
            res['backup'] = os.path.basename(b) if b else None
            if b:
    ...
    if hasattr(recipe, 'preview'):
        try:
            ...
            res['preview'] = [os.path.relpath(p, root).replace('\\', '/')
                              for p in recipe.preview(ctx, unit)]
        except Exception:
            pass
    return res
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            if p == '/api/backups':
                ...
                d = os.path.join(out_root(rid, unit), '_backup')
                names = sorted(os.listdir(d), reverse=True) if os.path.isdir(d) else []
                return self._send(200, {'list': names[:20]})
            if p == '/api/proto':
        ...
        try:
            ...
            if u.path == '/api/promote':
                ...
                def go():
                    ...
                    return ('rules.json 으로 %d항목 옮김 (그대로 %d)\n\n%s'
                            % (len(r['moved']), len(r['same']), runner.compare(rid)))
    ...
    def _recipe_info(self, rid):
        ...
        try:
            ...
            units = []
        return {'id': rid, 'name': getattr(m, 'NAME', rid),
