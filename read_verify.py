...
def verify(n, out=None):
    ...
    print('[verify] %s  <- %s' % (('lesson%02d' % n) if isinstance(n, int) else n, OUTDIR))
    ...
    made |= {read_gen.all_popup() + '.html'}
    _intro = read_gen.intro_page()          # 들머리 쪽이 없는 단원도 있다
    if _intro:
        made.add(_intro + '.html')
