...
def write_scrolls(n, data, gen_dir):
    ...
    for page in read_gen.ORDER:
        ...
        lines.append(' },')
    lines.append('}')
    _dump(os.path.join(gen_dir, 'scrolls%s.py' % n),
...
def write_popscroll(n, kor1, allv, gen_dir):
    ...
    for i in range(0, len(items), 3):
        L.append(' ' + ' '.join("'%s': %d," % kv for kv in items[i:i + 3]))
    L.append('}')
    _dump(os.path.join(gen_dir, 'popscroll%s.py' % n),
          '# read_measure.py 가 만든 값 — 직접 고치면 다음 측정 때 덮어쓴다.\n'
...
def write_krlayout(n, kr, gen_dir):
    ...
    for page in read_gen.ORDER:
        ...
        L.append(' ],')
    L.append('}')
    _dump(os.path.join(gen_dir, 'krlayout%s.py' % n),
          '# read_measure.py 가 만든 값 — 직접 고치면 다음 측정 때 덮어쓴다.\n'
...
def measure(n, gen_dir=None, headless=True):
    ...
    for page in pages:
        ...
        # 제목 이미지가 없는 쪽(7·8단원은 첫 쪽에만 제목이 있다)은 제목 해석도 없다
        arr = [p['title']['kr']] if p['title'] else []
    ...
    for m in ('scrolls%s' % n, 'popscroll%s' % n, 'krlayout%s' % n):
        if m in sys.modules:
