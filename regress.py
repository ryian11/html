...
def measure_ready(unit):
    """브라우저 없이 '측정이 끝까지 가는가'를 본다.

    회귀 검사는 생성 단계만 다시 돌리므로 측정 단계는 검사 밖이었다.
    제목 없는 쪽에서 측정이 멈추던 결함이 오래 안 잡힌 것이 그 때문이다.

    같은 로직을 여기에 베껴 두면 진짜 코드가 바뀌어도 못 잡는다. 그래서
    **read_measure.measure() 를 그대로 부르고** 브라우저만 대역으로 세운다.
    잰 값은 임시 폴더로 보내므로 실제 파일은 건드리지 않는다.
    돌려주는 값: 탈이 난 까닭 (빈 목록이면 탈 없음)
    """
    import runner, types, contextlib
    runner.load_recipe          # 단원자료 폴더를 파이썬 경로에 올려 둔다
    import read_measure as RM
    ...
    class _Page(object):
        ...
        def evaluate(self, js, arg=None):
            if js is RM.JS_READY:
                return None
            if js is RM.JS_BODY_SCROLL:
                return {'v': {}, 'contH': 1200, 'MAX': 500, 'groups': []}
            if js is RM.JS_TITLEWORDS:
                return None
            if js is RM.JS_KR:
                return {'out': list(arg[0]), 'dbg': [], 'W': 1000}
            if js is RM.JS_POP_SCROLL:
                return {'v': {}, 'H': 589, 'MAX': 300}
            raise AssertionError('모르는 스크립트')
    ...
    class _PW(object):
        chromium = type('C', (), {'launch': lambda self, **k: _Br()})()
    ...
    keep = {k: sys.modules.get(k) for k in ('playwright', 'playwright.sync_api')}
    ...
    tmp = tempfile.mkdtemp(prefix='measure_check_')
    try:
        with open(os.devnull, 'w') as dn, contextlib.redirect_stdout(dn):
            RM.measure(as_unit(unit), gen_dir=tmp, headless=True)
        return []
...
def check(rid, units=None):
    ...
    for unit in units:
        ...
        try:
            ...
            shutil.rmtree(tmp, ignore_errors=True)
        try:
            bad = measure_ready(unit)
        except Exception as e:
            bad = ['측정 점검 자체가 실패: %s' % e]
        if bad:
            ok = False
            lines.append('   !! 측정 준비에서 걸림')
